  ...
  new googleStrategy(
    ...
    (accessToken, refreshToken, profile, cb) => {
      ...
      db.query(
        ...
        [profile.id],
        (err, user) => {
          console.log("user from response where google Id", user);
          if (err) {
            cb(err, false);
          }
          if (!err && user.length != 0) {
            ...
            db.query(
              "insert into users set userName = ?, googleId = ?, userEmail = ?, userImg = ?",
              [profile.displayName, profile.id, profile.emails[0].value, profile.photos[0].value],
              (err, userAdded) => {
                if (err) {
